...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  void _validateFormState() {
    ...
    if (_currentOrder.status == 'PENDING') {
      ...
      isValid = _currentOrder.isPickupFulfillment || _hasValidSelectedDriver;
    } else {
      isValid = true;
    }
  ...
  Future<void> _handleDispatchOrder() async {
    if (!_hasValidSelectedDriver) {
      AppDialog.showToast(
        context,
        'Please select an available delivery driver',
        isError: true,
      );
      return;
    }
    ...
    switch (_currentOrder.status) {
      ...
      case 'COOKING':
        if (_currentOrder.isPickupFulfillment) {
          ...
          onPressed = (_isUpdating || !_hasValidSelectedDriver)
              ? null
